...
const DoctorHomeScreen = ({ navigation }: any) => {
  ...
  const formatMetricValue = (metric: {
    ...
    if (!Number.isNaN(romNum)) {
      return t('doctorHome.metricRom', { value: romNum.toFixed(1) });
    }

    if (!Number.isNaN(velNum)) {
      return t('doctorHome.metricVelocity', { value: velNum.toFixed(2) });
    }

    return t('doctorHome.noValue');
  };
    ...
    <>
      ...
      {dashboardKpis !== null && (
        <View style={styles.statsGrid}>
          <StatCard
            ...
            label={t('doctorHome.totalPatients')}
            color={colors.purple[500]}
          />
          <StatCard
            icon="fitness"
            value={dashboardKpis.activePatients}
            label={t('doctorHome.active')}
          ...
          <StatCard
            ...
            value={
              ...
              !Number.isNaN(dashboardKpis.avgProgress)
                ...
                : t('doctorHome.noValue')
            }
            label={t('doctorHome.avgProgress')}
      ...
      {metricsSummary.length > 0 && (
        <View style={[styles.quickView, { backgroundColor: colors.card }]}>
          <Text style={[styles.quickViewTitle, { color: colors.text }]}>
            {t('doctorHome.latestMetrics')}
          </Text>
          {metricsSummary.slice(0, 5).map((metric, idx) => (
            <View key={idx} style={styles.quickViewRow}>
              <Text
                style={[styles.quickViewName, { color: colors.text }]}
                numberOfLines={1}
              >
                {metric.patientName}
              </Text>
              <Text
                style={[styles.quickViewMeta, { color: colors.textSecondary }]}
              >
                {metric.joint} {metric.side ? `(${metric.side})` : ''} -{' '}
                {formatMetricValue(metric)}
              </Text>
              {metric.date ? (
                <Text
                  style={[styles.quickViewComment, { color: colors.textSecondary }]}
                  numberOfLines={1}
                >
            ...
            <View key={idx} style={styles.quickViewRow}>
              ...
              >
                {activity.type === 'session'
                  ? t('doctorHome.sessionType')
                  : t('doctorHome.feedbackType')}{' '}
                - {activity.label}
              </Text>
              {activity.date ? (
                <Text
      ...
      {trends !== null && (
        <View style={[styles.quickView, { backgroundColor: colors.card }]}>
          <Text style={[styles.quickViewTitle, { color: colors.text }]}>
            {t('doctorHome.trendsTitle')}
          </Text>
          <View style={[styles.quickViewRow, { marginBottom: 4 }]}>
            <Text style={[styles.quickViewMeta, { color: colors.textSecondary }]}>
              {t('doctorHome.avgPain', { value: trends.avgPain.toFixed(1) })}
            </Text>
          </View>
          <View style={[styles.quickViewRow, { marginBottom: 4 }]}>
            <Text style={[styles.quickViewMeta, { color: colors.textSecondary }]}>
              {t('doctorHome.avgFatigue', {
                value: trends.avgFatigue.toFixed(1),
              })}
            </Text>
          </View>
          <View style={[styles.quickViewRow, { marginBottom: 0 }]}>
            <Text style={[styles.quickViewMeta, { color: colors.textSecondary }]}>
              {t('doctorHome.avgDifficulty', {
                value: trends.avgDifficulty.toFixed(1),
              })}
            </Text>
          </View>
        </View>
      ...
      {(() => {
        const confirmedPatients = doctorPatientsItems.filter(
          (x) => x.type === 'patient'
        );
        const patientsNeedingAttention = confirmedPatients.filter((item) => {
          const patient = patients[item.id];
          const hasRecoveryProcess = !!(
            patient?.recovery_process && patient.recovery_process.length > 0
          );
          const hasAssignedExercises = !!(
            assignedExercises[item.id] && assignedExercises[item.id].length > 0
          );
        ...
        return (
          <View style={[styles.quickView, { backgroundColor: colors.card }]}>
            <Text style={[styles.quickViewTitle, { color: colors.text }]}>
              {t('doctorHome.patientsNeedingAttention')}
            </Text>
            {patientsNeedingAttention.slice(0, 5).map((item) => (
              <View key={item.id} style={styles.quickViewRow}>
                ...
                <Text
                  style={[styles.quickViewMeta, { color: colors.textSecondary }]}
                >
                  {t('doctorHome.noExercisesAssigned')}
                </Text>
              </View>
            ))}
          </View>
          ...
          {latestFeedback.slice(0, 5).map((fb) => (
            <View key={fb.id} style={styles.quickViewRow}>
              ...
              <Text
                style={[styles.quickViewMeta, { color: colors.textSecondary }]}
              >
                {t('doctorHome.feedbackSummary', {
                  pain: fb.pain ?? t('doctorHome.noValue'),
                  fatigue: fb.fatigue ?? t('doctorHome.noValue'),
                  difficulty: fb.difficulty ?? t('doctorHome.noValue'),
                })}
              </Text>
              {fb.comments ? (
                <Text
    ...
    return (
      <View style={[styles.safeArea, { backgroundColor: colors.background }]}>
        <View style={[styles.errorContainer, { backgroundColor: colors.card }]}>
          ...
          >
            <Text style={styles.retryButtonText}>{t('doctorHome.tryAgain')}</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
        ...
        ListEmptyComponent={
          <View style={[styles.emptyContainer, { backgroundColor: colors.card }]}>
            ...
            <Text style={[styles.emptyTitle, { color: colors.text }]}>
              {t('doctorHome.emptyTitle')}
            </Text>
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>
              {t('doctorHome.emptyDescription')}
            </Text>
          </View>
        }
      />
